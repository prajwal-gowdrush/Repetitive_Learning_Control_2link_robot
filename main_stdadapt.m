%% MAINSCRIPT STANDARD ADAPTIVE CONTROLLER
clc; clear; close all;

global K1 alpha gamma

%%%%%%%%%%%%%%%%%%%Tunable Controller Parameters%%%%%%%%%%%%%%%%%%%%%%%%%%%
K1=10; %Constant multiplying 'r' in the control law
alpha=1; %Constant in the definition of the error signal 'r'
gamma=diag([300; 2; 20; 80 ;20; 15; 15]);%Learning rate matrix in the adaptive update law
...
p1hat0=8; p2hat0=0.7; p3hat0=0.5; fd1hat0=8; fd2hat0=2; fs1hat0=2; fs2hat0=1;
thetahat0=[p1hat0;p2hat0;p3hat0;fd1hat0;fd2hat0;fs1hat0;fs2hat0];
...
[time,X_Sol]=ode113(@ode_stdadapt,tspan,X0,options);
...
p1error= p1-p1hat; p2error= p2-p2hat; p3error= p3-p3hat; fd1error= fd1-fd1hat; fd2error= fd2-fd2hat; fs1error= fs1-fs1hat; fs2error= fs2-fs2hat;
r1=e1dot+alpha*e1;
r2=e1dot+alpha*e2;
...
y2thetahat_1= y2thetahat_1 + fs1hat.*sign(q1dot);
...
y2thetahat_2=y2thetahat_2 + fs2hat.*sign(q2dot);
...
% figure;
subplot(2,2,3);
plot(time,p1hat,'r',time,p2hat,'b',time,p3hat,'k',time,fd1hat,'m',time,fd2hat,'g',time,fs1hat,'r--',time,fs2hat,'b--','LineWidth',1.5);
xlabel('Time');
...
ax.FontSize = 16;
ax.LineWidth = 1.4;
leg2 = legend('$\widehat{p}_1$','$\widehat{p}_2$','$\widehat{p}_3$','$\widehat{f}_{d1}$','$\widehat{f}_{d2}$','$\widehat{f}_{s1}$','$\widehat{f}_{s2}$');
set(leg2,'Interpreter','latex');
hold on;

% figure;
subplot(2,2,4);
plot(time,p1error,'r',time,p2error,'b',time,p3error,'k',time,fd1error,'m',time,fd2error,'g',time,fs1error,'r--',time,fs2error,'b--','LineWidth',1.5);
xlabel('Time');
...
ax.FontSize = 16;
ax.LineWidth = 1.4;
leg2 = legend('$p_1-\widehat{p}_1$','$p_2-\widehat{p}_2$','$p_3-\widehat{p}_3$','$f_{d1}-\widehat{f}_{d1}$','$f_{d2}-\widehat{f}_{d2}$','$f_{s1} - \widehat{f}_{s1}$','$f_{s2} - \widehat{f}_{s2}$');
set(leg2,'Interpreter','latex');
hold on;
